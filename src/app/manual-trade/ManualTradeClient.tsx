...
function generateManualTradeId() {
  return `MANUAL-${Date.now()}`;
}

const initialForm: ManualTradeForm = {
  symbol: "",
  direction: "Long",
  quantity: "",
  entryPrice: "",
...
export default function ManualTradeClient() {
  ...
  const [form, setForm] = useState<ManualTradeForm>(initialForm);
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        fyersTradeId: generateManualTradeId(),
      ...
      const res = await fetch("/api/sanity/save-manual-trade", {
        method: "POST",
        headers: {
      ...
      setTimeout(() => {
        router.push(`/trade-details/${data.tradeId}`);
      }, 900);
    } catch (err) {
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-8 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl shadow-[0_0_40px_rgba(34,211,238,0.06)]">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Enter your trade details manually and save them to your journal.
          </p>
        </div>
